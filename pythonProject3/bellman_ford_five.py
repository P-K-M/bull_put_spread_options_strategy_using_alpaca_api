...
base_currency = "USD"  # Set the base currency for the account
...
class App:
    ...
    def get_bid_ask(self, symbol):
        ...
        cursor = self.db_connection.cursor()
        query = """
            SELECT bid, ask, timestamp FROM currency_prices
            WHERE symbol = %s
            ORDER BY timestamp DESC LIMIT 1
        """
        cursor.execute(query, (symbol,))
        ...
        if result:
            return result[0], result[1]  # Return bid and ask
        else:
            ...
            for j in range(self.vertices):
                if i == j:
                    ...
                    if symbol not in currency_data:
                        ...
                        currency_data[symbol] = (bid, ask)
                    else:
                        bid, ask = currency_data[symbol]

                    # Adjust for spread (bid-ask spread impacts exchange rate efficiency)
                    spread_adjusted_price = (bid + ask) / 2  # Midpoint adjusted for spread
                    data = spread_adjusted_price if spread_adjusted_price else 1
    ...
    def calculate_transaction_costs(self, cycle):
        """
        Calculate transaction costs for a cycle.
        """
        total_cost = 0
        currency_conversion_fee = 0.005  # 0.5% currency conversion fee
        gslo_premium = 0.00015  # $0.00015 per unit for guaranteed stop-loss order premium
        slippage_factor = 0.0001  # 0.01% expected slippage

        prec = cycle[-1]
        for i in cycle[-2::-1]:
            bid, ask = self.get_bid_ask(f"{self.tickers[prec]}{self.tickers[i]}.a")

            # Spread cost
            spread_cost = (ask - bid)

            # Slippage cost
            slippage_cost = slippage_factor * ((bid + ask) / 2)

            # Add costs to total
            total_cost += spread_cost + slippage_cost + gslo_premium

            # Apply currency conversion fee if applicable
            if self.tickers[prec] != self.account_base_currency:
    ...
    def Bellman_Ford(self):
        ...
        if not Neg_cycles:
            ...
            for neg_cycle in Neg_cycles:
                ...
                transaction_costs = self.calculate_transaction_costs(neg_cycle)
                net_profit = profit - transaction_costs

                # Log all negative cycles with their adjusted profit
                cycle_str = ' --> '.join([self.tickers[i] for i in neg_cycle[::-1]])
                print(f"\nCycle detected: {cycle_str}\nProfit: {profit}, Net Profit (after costs): {net_profit}")
                logging.info(f"Cycle detected: {cycle_str}; Profit: {profit}, Net Profit: {net_profit}")

                # Collect profitable cycles
                if net_profit >= 1.0001:  # Ensure net profit exceeds an acceptable threshold
                    profitable_cycles.append((cycle_str, net_profit))
            ...
            if profitable_cycles:
                ...
                cycle_str, net_profit = most_profitable_cycle
                message = f"Cycle: {cycle_str}; Net Profit: {net_profit}"
