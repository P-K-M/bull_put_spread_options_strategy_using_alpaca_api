    ...
    def build_graph(self):
        for i in range(self.vertices):
            for j in range(self.vertices):
                if i == j:
                    data = 1  # The diagonal is always equal to 1
                else:
                    symbol = f"{self.tickers[i]}{self.tickers[j]}.a"  # Append ".a" to each currency pair symbol
                    data = self.get_midpoint(symbol)
    ...
    def Bellman_Ford(self):
        ...
        if not Neg_cycles:
            ...
            for neg_cycle in Neg_cycles:
                logging.info(f"Found negative cycle: {' --> '.join([self.tickers[i] for i in neg_cycle[::-1]])}")
