...
def get_tickers():
    ...
    start_date = pd.to_datetime("2024-08-01").tz_localize('America/New_York')
    end_date = pd.to_datetime("2024-09-25").tz_localize('America/New_York')
...
def run(tickers, market_open_dt, market_close_dt):
    ...
    for ticker in tickers:
        ...
        # Get data for the specified symbols
        request_params = StockBarsRequest(
            symbol_or_symbols=ticker, timeframe=TimeFrame.Day, start=datetime.datetime(2024, 5, 1),
    ...
    def run_wss_client():
        ...
        async def handle_second_bar(data):
            ...
            if (
                ...
                position = positions.get(symbol, 0)
                if position == 0:
                    print(f"Skipping check for {symbol} because the position is equal to 0.")

                else:
                    print(f"Checking Sell for {symbol} with position quantity: {position}")
