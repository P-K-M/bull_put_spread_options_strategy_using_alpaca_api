...
class App(Main):
    ...
    def get_midpoint_price(self, contract):
        ...
        rounded_price = None  # Initialize rounded_price
        ...
        for i in range(self.vertices):
            for j in range(self.vertices):
                if i == j:
                    ...
                    # Fetch midpoint price for the currency pair
                    contract = self.currency_pairs[i]
                    midpoint_price = self.get_midpoint_price(contract)

                    # Check if midpoint_price is available
                    if midpoint_price is not None:
                        # Use midpoint_price for building the graph
                        data = midpoint_price
                    else:
                        # Set a default value or handle it according to your requirements
                        print(f"No midpoint price available for {contract}. Using default value.")
            ...
            for i in range(self.vertices):
                for j in range(self.vertices):
                    if i == j:
                        ...
                        self.get_historical_data([contract])  # Fetch historical data for the current contract only
                        ...
                        if req_id in self.market_data:
                            ...
                            if market_data.Bid is not None and market_data.Ask is not None:
                                ...
                                if portfolio_value is not None:
                                    ...
                                    rounded_price = midpoint_price  # Assigning midpoint price
                                    break  # Exit the loop once midpoint price is obtained

                        if rounded_price is not None:
                            break  # Exit the while loop if midpoint price is obtained

                        # Assigning data for graph
                        data = rounded_price if rounded_price is not None else 1
