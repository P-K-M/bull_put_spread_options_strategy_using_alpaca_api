...
api_key_id = 'PK050FKJL22JS3NZNN0X'
api_secret = 'O4YtSakfIUfiw1nJT0FWKvDdn18nSYePVlVOswEB'
...
def get_tickers():
    ...
    symbols = ["AVAX/USD", "BAT/USD", "CRV/USD", "DOT/USD",
               "GRT/USD", "LINK/USD", "UNI/USD", "USDC/USD",
               "USDT/USD", "SUSHI/USD", "XTZ/USD"]

    # Convert the list of symbols to a set to remove duplicates, then back to a list
...
def find_stop(current_value, minute_history, now):
    series = minute_history['low'][-100:].dropna()

    # Filter the 'index' to keep only Datetime objects without altering the length
    is_datetime = [isinstance(item, pd.Timestamp) for item in minute_history['low'].index]

    # Ensure the boolean index matches the length of 'series' by using slicing
    series = series[is_datetime[:100]]

    # Convert 'now' to a Pandas Timestamp object with UTC time zone using tz_convert
    now = pd.Timestamp(now).tz_convert('UTC')

    # Ensure 'series.index' is a DatetimeIndex
    series.index = pd.DatetimeIndex(series.index)

    # Filter 'series' based on matching dates with 'now'
    series_slice = series[series.index.floor('D') == now.floor('D')]
...
def run(tickers, market_open_dt, market_close_dt):
    ...
    trading_stream = TradingStream('PK050FKJL22JS3NZNN0X', 'O4YtSakfIUfiw1nJT0FWKvDdn18nSYePVlVOswEB', paper=True)

    wss_client = CryptoDataStream('PK050FKJL22JS3NZNN0X', 'O4YtSakfIUfiw1nJT0FWKvDdn18nSYePVlVOswEB')
        ...
        async def handle_second_bar(data):
            ...
            if (
                ...
                if position == 0:
                    ...
                    print(f"Checking negative MACD check for {symbol}")
                    ...
                    if data.close <= stop_prices[symbol]:
                        ...
                        print("Stop Price:", stop_target_prices[symbol])
                    ...
                    if data.close >= target_prices[symbol] and Histogram.iloc[-1] <= 0:
                        ...
                        print("Target Price:", stop_target_prices[symbol])
...
if __name__ == "__main__":
    ...
    market_open = today.replace(hour=8, minute=00, second=0, microsecond=0)

    market_open = market_open.astimezone(nyc)
    market_close = today.replace(hour=1, minute=30, second=0, microsecond=0)
