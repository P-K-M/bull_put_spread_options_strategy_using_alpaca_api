...
API_KEY = "PKBF1MC1XHCCS61F0TZE"
SECRET_KEY = "hrUCxQ6PkE6zwm7JqWAgYGBeeIoP2d9JZWpBEOJu"
...
class BullPutSpread:
    ...
    # ==============================================================================
    # SECTION 4: Implied Volatility, Greeks, and Technical Indicators
    # ------------------------------------------------------------------------------
    ...
    def check_option_conditions(self, candidate: dict, label: str, criteria: dict) -> bool:
        ...
        """
        # Map label to leg type used in criteria
    ...
    def find_bull_put_spread(
        ...
        """
        # --- Step 1: Standardize option data ---
        valid_candidates = []
    ...
    def score_spread(self, short_put: dict, long_put: dict) -> float:
        ...
        try:
            ...
            # Theta adjustment: Factor in theta (time decay benefit)
            ...
            #Normalize bid/ask spreads and OI
    ...
    def check_buying_power(self, short_put: Dict[str, Any], long_put: Dict[str, Any]) -> bool:
        ...
        """
        try:
            # --- Step 1: Retrieve account info ---
            account = self.trading_client.get_account()
            buying_power = float(account.buying_power)
            portfolio_value = float(account.portfolio_value)

            # === Log buying power info ===
            self.logger.info(f"[ACCOUNT] Buying Power Limit: ${buying_power:,.2f} | Portfolio Value: ${portfolio_value:,.2f}")

            # --- Step 2: Define risk budget ---
    ...
    def roll_rinse_bull_put_spread(
        ...
        def extract_underlying_from_option_symbol(option_symbol: str) -> str:
            """
            Extract the underlying ticker symbol from an option OCC-format symbol.
            Example: AAPL250919P00150000 → AAPL
            """
            import re
        ...
        def latest_mid(symbol: str) -> Optional[float]:
            """
            Best-effort mid-price calculation for an option symbol using latest bid/ask.
        ...
        def resolve_entry_price(leg: Dict[str, Any]) -> Optional[float]:
            """
            Resolve the entry price for an option leg using:
            1. Recorded execution prices in strategy state.
            2. Broker-reported average entry price (fallback).
            3. Current mid-price (last resort).
            """
        ...
        # -------------------------------
        # Main Roll/Rinse Evaluation Logic
        # -------------------------------
        try:
            ...
            # --- Step 1: Calculate Entry Net Credit ---
            ...
            # Define profit targets dynamically based on entry credit
            ...
            # Relaxed profit target (70% of strict target, only active Wed–Fri)
            relaxed_target = round(target_profit * 0.7, 2)

            # --- Step 2: Calculate Current PnL ---
            try:
                positions = self.trading_client.get_all_positions()
            ...
            # --- Step 3: Track Holding Time & Exit Windows ---
            days_open = 0
            ...
            # --- Step 4: Risk Metrics (Delta & Vega) ---
            ...
            # Friday-only hard/soft stop-loss conditions
            ...
            # Roll logic (only on Friday, if under pressure and losing)
            if friday_only:
    ...
    def sync_with_alpaca(self):
        ...
        def parse_option_symbol(sym: str) -> dict:
            """
            Parse Alpaca option symbol into components (underlying, expiry, type, strike).
            Example: AAPL250919P00180000 → AAPL, 2025-09-19, Put, 180.0
            """
        ...
        def run_wss_client():
            async def handle_second_bar(data):
                ...
                try:
                    ...
                    if market_open + timedelta(minutes=15) <= ts < market_open.replace(hour=15, minute=0):
                        # --- Refresh live spreads before screening ---
                        ...
                        # --- Skip if symbol already has an open leg ---
                        ...
                        if last_check is None or (ts - last_check).total_seconds() >= 300:
                            ...
                            # === Step 2: Technical score (RSI, MACD, SMA, ADX, Volume) ===
                            score = sum([
                            ...
                            # === Step 3: Fetch options chain (PUTs, 8–14 DTE) ===
                            ...
                            # Retry with relaxed criteria if strict fails
                            if not all(spread):
                            ...
                            try:
                                ...
                                # --- Account checks & sizing calculations ---
                                try:
                                    account = self.trading_client.get_account()
                                except Exception as e:
                                ...
                                # Be consistent: set 0.1 for 10% or fix comment
                                per_trade_cap = portfolio * 0.1  # Max 5% of portfolio per trade
                    ...
                    # === Portfolio-Level Profit Exit (end-of-week safeguard) ===
